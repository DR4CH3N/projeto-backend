...
<body class="">
      ...
      if (empty($selecionador) || empty($validacao)) {
        echo "desculpa, não pudemos validar seu pedido!";
      } else {
        /* aqui ele vai verificar se o token que voce recebeu no email e o mesmo da pagina e vai verificar se o mesmo é hexadecimal */
        if (ctype_xdigit($selecionador) !== false && ctype_xdigit($validacao) !== false ) { ?>

          <!-- área de resetar a senha -->
        <form action="inc/reset-password.inc.php" method="POST">
          <input type="hidden" name="selecionador" value="<?php $selecionador ?>">
          <input type="hidden" name="validacao" value="<?php $validacao ?>">
          ...
          <div class="text-center text-lg-start mt-4 ">
            <button type="submit" name="reset-password-submit" id="reset-senha-submit" class="btn btn-primary col-12 btn-lg"
              >Confirmar senha nova</button> 
          </div>
        </form>
